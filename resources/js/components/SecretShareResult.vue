...
import { AlertTriangle, Check, ShieldAlert, Trash2 } from 'lucide-vue-next';
import { ref } from 'vue';
    ...
    <section class="mx-auto w-full max-w-2xl">
            ...
            <CardHeader class="space-y-4 border-b border-border/60">
                <div class="flex items-start justify-between gap-4">
                    <div class="space-y-2">
                        ...
                        <h1 class="text-2xl font-semibold tracking-tight sm:text-3xl" aria-live="polite">Your secure link is ready</h1>
            ...
            <CardContent class="space-y-6 pt-6">
                <div class="space-y-2">
                    <div class="flex items-center justify-between gap-3">
                        ...
                        <span class="text-right text-xs text-muted-foreground">Expires {{ new Date(secret.expiresAt).toLocaleString() }}</span>
                    </div>
                    <div class="flex gap-2">
                        <Input
                            id="share-link"
                            :model-value="secret.shareUrl"
                            readonly
                            class="min-w-0 font-mono text-xs"
                            aria-describedby="share-link-help"
                            @focus="selectShareUrl"
                        />
                        <CopyButton :text="secret.shareUrl" label="Copy shareable URL" />
                    </div>
                    <p id="share-link-help" class="text-xs text-muted-foreground">Focus the field to select the full link.</p>
                </div>
                ...
                <div v-if="secret.password" class="space-y-2 rounded-lg border border-primary/20 bg-primary/5 p-4">
                    ...
                    <div class="flex gap-2">
                        ...
                        <CopyButton :text="secret.password" label="Copy secret password" />
                    </div>
                </div>

                <div class="space-y-2 rounded-lg border border-amber-500/30 bg-amber-500/5 p-4" role="note">
                    ...
                    <div class="flex gap-2">
                        <Input :model-value="secret.revocationToken" readonly type="text" class="min-w-0 font-mono text-xs" />
                        <CopyButton :text="secret.revocationToken" label="Copy private revocation token" />
                    </div>
                </div>
                ...
                <Alert v-if="status === 'revoked'" aria-live="polite">
        ...
        <AlertDialog v-model:open="showRevokeDialog">
            <AlertDialogPortal>
                ...
                <AlertDialogContent
                    class="fixed left-1/2 top-1/2 z-50 grid w-[calc(100%-2rem)] max-w-md -translate-x-1/2 -translate-y-1/2 gap-6 rounded-xl border bg-card p-6 text-card-foreground shadow-2xl outline-none"
                >
                    <div class="flex items-start gap-3">
                        ...
                        <div class="space-y-2">
                            ...
                            <AlertDialogDescription> The share link will stop working immediately. This cannot be undone. </AlertDialogDescription>
                        </div>
                    </div>
                    <div class="flex flex-col-reverse gap-2 sm:flex-row sm:justify-end">
                        ...
                        <AlertDialogAction as-child>
                            <Button type="button" variant="destructive" @click="confirmRevoke(secret)">Revoke secret</Button>
                        </AlertDialogAction>
                    </div>
                </AlertDialogContent>
            </AlertDialogPortal>
